...
const tutorials = [
  { 
    ...
    steps: [
      {
        name: '底層十字 (Cross)', 
        desc: '這是復原的基石。首先，在頂面（通常是黃色中心）周圍尋找帶有白色的邊塊，將它們像花瓣一樣圍繞黃色中心。接著，將這些「白色花瓣」側邊的顏色，與對應的側面中心塊對齊，然後轉動 180 度送到底部。完成後，底部會形成一個完美的白色十字，且側邊顏色皆與中心一致。'
      }, 
      {
        name: '底層角塊 (First Layer)', 
        desc: "尋找帶有白色的角塊。觀察角塊上另外兩個顏色，將角塊轉動到這兩個顏色對應的中心塊之間。使用基本公式（如 R U R' U'）將角塊放入底層。重複此動作，直到四個角塊都歸位，此時第一層（包含底面）將完全復原，側面會呈現倒 T 字型。",
        alg: "R U R' U'",
        setupAlg: "U R U' R'"
      }, 
      {
        name: '第二層邊塊 (Second Layer)', 
        desc: '將視線移至頂層，尋找「不包含黃色」的邊塊。將該邊塊側面的顏色與對應的中心塊對齊，形成一個倒 T 字。觀察邊塊頂部的顏色，判斷它應該放入左邊還是右邊的槽位，並使用對應的左/右公式將其放入。完成四個邊塊後，前兩層 (F2L) 即告完成。',
        alg: "U R U' R' U' F' U F",
        setupAlg: "F' U' F U R U R' U'"
      }, 
      {
        name: '頂層十字 (Top Cross)', 
        desc: "觀察頂層黃色邊塊的形狀。你可能會看到：一個點、一個小拐角（L型）、一條直線（一字型），或是已經完成的十字。根據不同的形狀，使用特定的公式（如 F R U R' U' F'）來翻轉邊塊，直到頂面出現一個黃色十字。不用理會角塊的顏色。",
        alg: "F R U R' U' F'",
        setupAlg: "F U R U' R' F'"
      }, 
      {
        name: '頂面顏色 (OLL 基礎)', 
        desc: "現在要讓頂面全部變成黃色。觀察頂層四個角塊的黃色朝向。將魔方轉到特定的角度（例如讓一個黃色朝前的角塊放在左前角），然後重複使用基本公式（如 R U R' U R U2 R'），直到頂面完全被黃色填滿。",
        alg: "R U R' U R U2 R'",
        setupAlg: "R U2 R' U' R U' R'"
      }, 
      {
        name: '頂層角塊 (PLL 角塊)', 
        desc: '觀察頂層側面的角塊顏色。尋找是否有兩個相鄰角塊顏色相同的「車頭燈」。如果有，將車頭燈放在左側或後方，使用公式將所有角塊移動到正確位置。如果沒有車頭燈，隨意執行一次公式就會出現。完成後，四個面的角塊顏色都會對齊。',
        alg: "R U R' U' R' F R2 U' R' U' R U R' F'",
        setupAlg: "F R U' R' U R U R2 F' R U R U' R'"
      }, 
      {
        name: '頂層邊塊 (PLL 邊塊)', 
        desc: '最後一步！觀察頂層側面的邊塊。通常會有一個面的顏色已經完全連線，將那個面放在後方。觀察剩下三個邊塊需要順時針還是逆時針交換，使用最後的公式（如 U-Perm）將邊塊互相交換。恭喜你，完成整顆魔術方塊！',
  ...
  { 
    ...
    steps: [
      {
        name: '底層完成 (First Layer)', 
        desc: '2x2 沒有中心塊作為參考，因此任選一個帶有白色的角塊作為起點。尋找另一個帶有白色，且與起點角塊有共通顏色的角塊。使用 3x3 的底層角塊公式將其放入。確保放入後，底層側面的顏色是相連且一致的。',
        alg: "R U R' U'",
        setupAlg: "U R U' R'"
      }, 
      {
        name: '頂面翻色 (OLL)', 
        desc: '將魔方翻轉，讓未完成的面朝上。觀察頂層角塊的黃色朝向。這部分的邏輯與 3x3 的「頂面顏色」步驟完全相同，使用相同的公式將頂面四個角塊全部翻成黃色。',
        alg: "R U R' U R U2 R'",
        setupAlg: "R U2 R' U' R U' R'"
      }, 
      {
        name: '頂層排列 (PLL)', 
        desc: '觀察頂層側面的顏色。尋找是否有兩個相鄰顏色相同的角塊（車頭燈）。如果有，將其對齊底層顏色並放在左側或後方，使用 3x3 的角塊排列公式（如 T-Perm 或 Y-Perm）將其交換至正確位置，完成復原。',
        alg: "R U R' U' R' F R2 U' R' U' R U R' F'",
        setupAlg: "F R U' R' U R U R2 F' R U R U' R'"
      }
    ], 
  ...
  { 
    ...
    steps: [
      {
        name: '完成六面中心 (Centers)', 
        desc: '4x4 沒有固定的中心塊。首先，在任意面組合出四個白色的中心塊。接著，在對面組合出四個黃色的中心塊。然後，依序完成側面的藍、紅、綠、橘中心。務必牢記正確的顏色相對位置（例如：白上黃下時，藍的右邊是紅），否則最後無法復原。'
      }, 
      {
        name: '邊塊配令人 (Edge Pairing)', 
        desc: '尋找兩個顏色完全相同的邊塊。透過轉動外層，將它們移動到同一個軌道上（通常是一左一右）。轉動中間層將它們結合，然後替換入一個未完成的廢邊，再將中間層轉回以修復中心。重複此動作，完成所有 12 組邊塊。',
        alg: "Uw' R U R' F R' F' R Uw",
        setupAlg: "Uw' R' F R F' R U' R' Uw"
      }, 
      {
        name: '3x3 階段復原', 
        desc: '現在，將配對好的邊塊視為 3x3 的單一邊塊，四個一組的中心視為 3x3 的單一中心。使用你熟悉的 3x3 層先法或 CFOP 進行復原。'
      }, 
      {
        name: '處理特殊情況 (Parity)', 
        desc: '在 3x3 階段，你可能會遇到 3x3 不可能出現的情況。如果頂層十字無法完成（單邊翻轉），這是 OLL Parity；如果最後只剩兩個邊塊互換，這是 PLL Parity。必須背誦並使用 4x4 專屬的長公式來解決這些奇偶校驗問題。',
        alg: "Rw U2 x Rw U2 Rw U2 Rw' U2 Lw U2 Rw' U2 Rw U2 Rw' U2 Rw'",
        setupAlg: "Rw U2 Rw' U2 Rw U2 Rw' U2 Lw' U2 Rw U2 Rw' U2 Rw U2 x' Rw'"
      }
    ], 
    videoId: 'KWOZHbDdOeo' 
  },
  { 
    ...
    steps: [
      {
        name: '完成六面中心 (Centers)', 
        desc: '5x5 有固定的中心點。以固定中心為基準，先將周圍的 8 個中心塊組合成一個 1x3 的長條，再將三個長條合併，完成一個 3x3 大小的中心區塊。依序完成白、黃，以及側面的四個中心。'
      }, 
      {
        name: '邊塊配對 (Edge Pairing)', 
        desc: '尋找三個顏色相同的邊塊（一個中邊，兩個翼邊）。利用中間層的轉動，將它們組合成一個完整的長條邊塊。這階段通常使用 Free Slice 技巧，在不破壞已配對邊塊的前提下，高效率地完成 12 組邊塊。'
      }, 
      {
        name: '3x3 階段復原', 
        desc: '將組合好的巨大邊塊與 3x3 大小的中心區塊，視為一顆標準的 3x3 魔術方塊。使用 3x3 的方法進行復原。'
      }, 
      {
        name: '處理特殊情況 (Parity)', 
        desc: '5x5 作為奇數階魔方，不會出現 PLL Parity。但如果在組邊的最後階段（L4E），發現剩下兩個邊塊無法正常配對（單邊翻轉），則需要使用 5x5 的 Edge Parity 公式來解決。',
        alg: "Rw U2 x Rw U2 Rw U2 Rw' U2 Lw U2 Rw' U2 Rw U2 Rw' U2 Rw'",
        setupAlg: "Rw U2 Rw' U2 Rw U2 Rw' U2 Lw' U2 Rw U2 Rw' U2 Rw U2 x' Rw'"
      }
    ], 
  ...
  { 
    ...
    steps: [
      {
        name: '白色星形 (Star)', 
        desc: '五魔方的結構類似 3x3，只是面數變多。首先在白色面尋找五個帶有白色的邊塊，將它們的側面顏色與對應的中心對齊，形成一個完美的白色五角星。'
      }, 
      {
        name: '底層與第二層 (F2L)', 
        desc: '使用與 3x3 相同的 F2L 概念。尋找帶有白色的角塊與對應的邊塊，將它們配對後放入白色星形周圍的五個槽位，完成底部的一個半球體。',
        alg: "R U R' U'",
        setupAlg: "U R U' R'"
      }, 
      {
        name: '側面延伸 (S2L)', 
        desc: '這是五魔方最耗時的階段。依序選擇側面的顏色（例如：藍、紅、綠、紫、黃），像蓋房子一樣，逐一完成各個顏色的星形與 F2L，逐步向上推進，直到只剩下最後一個頂面（通常是灰色）。'
      }, 
      {
        name: '頂層復原 (LL)', 
        desc: '最後的灰色頂面。使用類似 3x3 的方法：先完成灰色星形（邊塊方向），再調整邊塊位置，接著翻轉角塊的灰色朝上，最後排列角塊位置，完成整顆五魔方。',
        alg: "R U R' U R U2 R'",
        setupAlg: "R U2 R' U' R U' R'"
      }
    ], 
  ...
  { 
    ...
    steps: [
      {
        name: '角塊與中心 (Tips & Centers)', 
        desc: '金字塔最外側的四個小角塊（Tips）只會自己旋轉，不會改變位置。先將它們轉到與相鄰顏色一致。接著，尋找三個同色的中心塊（例如黃色），轉動大角，將這三個中心塊集中到同一個面上。'
      }, 
      {
        name: '完成底層 (First Layer)', 
        desc: '將剛才集中的黃色面朝下作為底面。在上方尋找帶有黃色的邊塊，將其側面顏色與中心對齊，使用簡單的「下下上上」公式將其放入底層。完成三個邊塊後，第一層即告完成。',
        alg: "R U R' U'",
        setupAlg: "U R U' R'"
      }, 
      {
        name: '頂層復原 (Last Layer)', 
        desc: '觀察剩下的三個邊塊。轉動頂層讓中心塊對齊。根據邊塊的排列狀態（例如：三個邊塊順時針輪換、兩個邊塊原地翻轉等），使用對應的基本公式（通常只有 4-5 種情況）將其歸位。',
        alg: "R U R' U R U R'",
  ...
  { 
    ...
    steps: [
      {
        name: '復原形狀 (Cube Shape)', 
        desc: 'Square-1 打亂後形狀會改變。第一步是透過特定的轉動步驟，將上下層的 8 個角塊與 8 個邊塊集中並配對，最終將其恢復成標準的正方體形狀。這是初學者最容易卡關的步驟。'
      }, 
      {
        name: '角塊分色 (CO)', 
        desc: '形狀復原後，目標是將顏色分開。首先處理角塊：將白色的角塊全部集中到頂層，黃色的角塊全部集中到底層（不需理會側面顏色）。'
      }, 
      {
        name: '邊塊分色 (EO)', 
        desc: '接著處理邊塊：使用公式將白色的邊塊全部交換到頂層，黃色的邊塊交換到底層。完成後，頂面會是全白（或全黃），底面則是另一種顏色。',
        alg: "/ (3,-3) /",
        setupAlg: "/ (-3,3) /"
      }, 
      {
        name: '排列歸位 (CP & EP)', 
        desc: '最後階段。先觀察上下層角塊的側面顏色，使用公式將角塊移動到正確位置（CP）。接著觀察邊塊，使用公式將邊塊互相交換至正確位置（EP）。若遇到上下層各剩兩個邊塊互換的情況，即為 Parity，需使用特殊公式解決。'
      }
    ], 
    videoId: 'xITr2WFqado' 
  },
  { 
    ...
    steps: [
      {
        name: '完成底層 (First Layer)', 
        desc: '斜轉的轉動軸在角落。任選一個顏色作為底面中心（例如白色）。尋找四個帶有白色的角塊，透過角塊的旋轉，將它們轉入底層，並確保角塊側面的顏色與相鄰角塊一致。'
      }, 
      {
        name: '頂層角塊翻轉 (Top Corners)', 
        desc: '觀察頂層四個角塊的黃色朝向。斜轉有一個非常核心的基本公式：「下下上上」（Sledgehammer）。根據黃色角塊朝向的不同模式（如兩個朝上、零個朝上），重複使用此公式將四個角塊翻轉至黃色朝上。',
        alg: "R' F R F'",
        setupAlg: "F R' F' R"
      }, 
      {
        name: '中心塊歸位 (Centers)', 
        desc: '角塊完成後，觀察剩餘未歸位的中心塊。將需要移動到頂面的中心塊放在後方（B面），再次使用「下下上上」公式，配合魔方的整體翻轉（y2），將所有中心塊復原。'
      }
    ], 
    videoId: 'I6132yshkeU' 
  },
                  ...
                  {(step as any).alg && (
                    <div className="mt-4 w-full max-w-xs bg-slate-800/50 rounded-xl p-3 border border-slate-700/50 shadow-inner">
                      <div className="aspect-square w-full relative rounded-lg overflow-hidden bg-slate-900/50">
                        <TwistyPlayerWrapper 
                          puzzle={(activeTutorial as any).id === '333' ? '3x3x3' : 
                                 (activeTutorial as any).id === '222' ? '2x2x2' :
                                 (activeTutorial as any).id === '444' ? '4x4x4' :
                                 (activeTutorial as any).id === '555' ? '5x5x5' :
                                 (activeTutorial as any).id === 'megaminx' ? 'megaminx' :
                                 (activeTutorial as any).id === 'pyraminx' ? 'pyraminx' :
                                 (activeTutorial as any).id === 'sq1' ? 'square1' :
                                 (activeTutorial as any).id === 'skewb' ? 'skewb' : '3x3x3'} 
                          alg={(step as any).alg} 
